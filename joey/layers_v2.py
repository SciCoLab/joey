...
def default_dim_allocator(count, name):
    global dim_index
    dims =[]
    for i in range(1,count+1):
       dims.append(SpaceDimension(name+"_d"+str(i)))
    return dims
...
class Conv_v2(Layer):
    ...
    def _allocate(self, kernel_size, input_size, name_allocator_func,
        ...
        result_width = (input_size[2]-kernel_size[2] + 2 *self._padding[1])//self._stride[1] + 1 

        # input data function
        I = Function(name="Input_F",shape=(input_size), dimensions =(default_dim_allocator(len(input_size),"input")), space_order= (0,self._padding[0],self._padding[0]), dtype=np.double) 

        # function for kernel
        K = Function(name="Kernel_F", shape=(kernel_size), dimensions =(default_dim_allocator(len(kernel_size),"kernel")), space_order=0, dtype=np.double)

        # Result for convolution
        R = Function(name="Result_F", shape=(input_size[0], no_of_kernels, result_height,result_width), dimensions =(default_dim_allocator(len(input_size),"result")), space_order=0, dtype=np.double)

        bias = Function(name="bias_F", shape=(kernel_size[0],), dimensions =(default_dim_allocator(1,"bias")), space_order=0, dtype=np.double)

        kernel_grad = Function(name="kgrad_%s" % name_allocator_func(),shape=(kernel_size), dimensions =(default_dim_allocator(len(kernel_size),"kernel_grad")), space_order=0, dtype=np.double)

        output_grad = Function(name="outgrad_%s" % name_allocator_func(),shape=(input_size), dimensions =(default_dim_allocator(len(input_size),"result_grad")), space_order=0, dtype=np.double)

        bias_grad = Function(name="bgrad_%s" % name_allocator_func(),shape=(kernel_size[0],), dimensions =(default_dim_allocator(1,"bias_grad")), space_order=0, dtype=np.double)           

        return (K, I, R, bias, kernel_grad, output_grad, bias_grad)
    ...
    def equations(self):
        r4, r3, r1, r2 = self._R.dimensions
        _, channels, kernel_height, kernel_width = self._kernel_size
        k4,_, _, _ = self._K.dimensions

        self._R.data [:]= 0
        off_sets = [x  for x in range(0,self._kernel_size[2])]
        ...
        w = sp.Matrix([self._K[(k4,*x)] for x in k_indices]) 

        r1_offsets = [(r1*self._stride[0]+x)-self._padding[0] for x in off_sets]
        r2_offsets = [r2*self._stride[1]+x -self._padding[1] for x in off_sets]
        i3_offsets = [x for x in off_sets_channels]


        r_indicies = product(i3_offsets,r1_offsets,r2_offsets)
        r = sp.Matrix([self._I[(r4,*x)] for x in r_indicies])

        sten = w.dot(r)

        eq = Eq(self._R[r4,r3,r1,r2], sten)
...
class MaxPooling(Pooling):
    ...
    def equations(self):
        if self._forward_tmp_constants is None:
            self._forward_tmp_constants = \
                [Constant(name="fw_tmp_c_%s" % alloc(), dtype=np.float64)]
    ...
    def backprop_equations(self, prev_layer, next_layer):
        ...
        stride_rows, stride_cols = self._stride
...
class FullyConnected(Layer):
    ...
    def _allocate(self, weight_size, input_size, name_allocator_func,
        ...
        bias_grid = Grid(shape=weight_size[0],
                         dimensions=(t1,))
        bias = Function(name="bias_%s" % name_allocator_func(), grid=bias_grid,
                        space_order=0, dtype=np.float64)

        kernel_grad = Function(name="kgrad_%s" % name_allocator_func(),
        ...
        bias_grad = Function(name="bgrad_%s" % name_allocator_func(),
