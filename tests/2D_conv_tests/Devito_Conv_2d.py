...
def conv_devito_2D (input,kernel,stride,padding, print_code = True):
    ...
    gridR = Grid(shape=(result_size,result_size), dimensions =(ConditionalDimension("r_d1",parent=i_1, factor = stride),ConditionalDimension("r_d2",parent=i_2, factor = stride)))
    ...
    r1_offsets = [(r1*stride+x) for x in off_sets]

    r2_offsets = [r2*stride+x for x in off_sets]
...
def conv_devito_2D_defaultStride(input,kernel,stride,padding, print_code = True):
    ...
    r1_offsets = [r1+x for x in off_sets]

    r2_offsets = [r2+x for x in off_sets]
