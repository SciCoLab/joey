...
def test_joey_pytorch_conv2d(input_size, kernel_size, padding, stride,
    ...
    result_torch = pytorch_conv_2d(input_data, kernel, padding, stride)
...
@pytest.mark.parametrize("input_size, kernel_size, padding, stride",
                         [((2, 3, 5, 5, 9), (6, 1, 3, 2), 2, 1),
                          ((1, 3, 21, 46, 50), (6, 5, 3, 7), 10, 3)])
def test_joey_pytorch_conv3d(input_size, kernel_size, padding, stride,
    ...
    result_torch = pytorch_conv_3d(input_data, kernel, padding, stride)
    ...
    result_joey = layer.execute(input_data.detach().numpy(), [
                                0]*len(kernel), kernel.detach().numpy())
